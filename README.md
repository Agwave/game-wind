...
- 💰 流水估算：中国区畅销榜排名 → 日流水量级区间（**仅供参考**，不是精确数据）
- 📝 每日报告落盘 `data/reports/YYYY-MM-DD.md`，快照存 `data/YYYY-MM-DD.json`
- 📲 企业微信机器人推送（按地区分条，超长自动截断；无重大变化默认不打扰；同一天不重复推送）
...
# 2b. 代码检查（可选，golangci-lint v2，配置见 .golangci.yml）
golangci-lint run ./...
...
```
30 9 * * * cd ~/ai-project/game-wind && ./bin/gamewind run >> logs/cron.log 2>&1
```
...
`config.local.yaml`（本地，不入库）— webhook 等敏感配置，覆盖 `config.yaml` 同名键。

## 游戏→公司映射表（data/games.yaml）

匹配优先级：**app_id > 精确名 > 包含名**。app_id 跨地区稳定（同一游戏美日韩是同一个 id），
...
- 备用源 `rss.applemarketingtools.com` 已迁移至新域名且 v2 接口 404，不采用
